...
#define TAG "msm_adreno_balanced: "
 ...
 */
u64 suspend_time_ms_balanced(void)
{
	...
	char *buf)
{
	...
	time_diff = suspend_time_ms_balanced();
	/*
...
void compute_work_load_balanced(struct devfreq_dev_status *stats,
				...
				u32 *flag)
{
	...
	*freq = stats.current_frequency;
	...
	compute_work_load_balanced(&stats, priv, devfreq);
	/*
...
static int tz_handler(struct devfreq *devfreq, unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_RESUME:
		spin_lock(&suspend_lock);
		suspend_time += suspend_time_ms_balanced();

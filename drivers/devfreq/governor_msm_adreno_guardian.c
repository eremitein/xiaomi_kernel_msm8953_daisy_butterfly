...
#define TAG "msm_adreno_guardian: "
 ...
 */
u64 suspend_time_ms_guardian(void)
{
	...
	char *buf)
{
	...
	time_diff = suspend_time_ms_guardian();
	/*
...
void compute_work_load_guardian(struct devfreq_dev_status *stats,
				...
				u32 *flag)
{
	...
	*freq = stats.current_frequency;
	...
	compute_work_load_guardian(&stats, priv, devfreq);
	/*
...
static int tz_handler(struct devfreq *devfreq, unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_RESUME:
		spin_lock(&suspend_lock);
		suspend_time += suspend_time_ms_guardian();
